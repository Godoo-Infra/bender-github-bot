...
def test_is_merge_bot_branch():
    assert is_merge_bot_branch("12.0-benderbot-merge-pr-100-by-toto-bump-patch")
    assert not is_merge_bot_branch("12.0-cabot-merge-pr-a100-by-titi-bump-no")
    assert is_merge_bot_branch("master-benderbot-merge-pr-100-by-toto-bump-no")


def test_make_merge_bot_branch():
    assert (
        make_merge_bot_branch("100", "12.0", "toto", "patch")
        == "12.0-benderbot-merge-pr-100-by-toto-bump-patch"
    )


def test_parse_merge_bot_branch():
    assert parse_merge_bot_branch("12.0-benderbot-merge-pr-100-by-toto-bump-patch") == (
        "100",
        "12.0",
        "toto",
        "patch",
    )
    assert parse_merge_bot_branch("12.0-benderbot-merge-pr-100-by-toto-bump-no") == (
        "100",
        "12.0",
        "toto",
        "nobump",
...
def test_merge_bot_branch_name():
    # benderbot-merge must not change, as other tools may depend on it:
    # branch protection patterns and CI filters key on it.
    # The rest of the branch name must be considered opaque and fit for the bot
    # needs only.
    assert "benderbot-merge" in make_merge_bot_branch("100", "12.0", "toto", "patch")
